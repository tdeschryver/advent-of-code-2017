...
[<Fact>]
let ``Count valid passphrases`` () =
  Assert.Equal(2, Day04.countValidPassphrasesPart1 "
  aa bb cc dd ee
  aa bb cc dd aa
  aa bb cc dd aaa")
...
[<Fact>]
let ``Part 1`` () =
  Assert.Equal(466, Day04.countValidPassphrasesPart1 "
...
[<Fact>]
let ``Count valid passphrases part 2`` () =
  Assert.Equal(3, Day04.countValidPassphrasesPart2 "
  abcde fghij
  abcde xyz ecdab
...
[<Fact>]
let ``Part 2`` () =
  Assert.Equal(251, Day04.countValidPassphrasesPart2 "
  pphsv ojtou brvhsj cer ntfhlra udeh ccgtyzc zoyzmh jum lugbnk
  vxjnf fzqitnj uyfck blnl impo kxoow nngd worcm bdesehw
  caibh nfuk kfnu llfdbz uxjty yxjut jcea
  qiho qif eupwww avyglnj nxzotsu hio lws
